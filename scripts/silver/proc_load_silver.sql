...
BEGIN
	BEGIN TRY
		...
		(SELECT cst_id,
				...
				CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single' -- Data Normalization and standardization
		...
		WHERE flag_last = 1 AND cst_id IS NOT NULL -- Handling Duplicates and null values
		);
		...
		SELECT prd_id,
			   ...
			   CASE UPPER(TRIM(prd_line)) 
					...
					WHEN 'T' THEN 'Toruing'
					ELSE 'n/a'
			   END AS prd_line, -- Data Normalization and Standardization
			   prd_start_dt,
			   DATEADD(day,-1,LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)) AS prd_end_dt -- Calculate the end_dt as the day before teh next Start date of that product
